...
public class NIOServer extends Thread {
    ...
    class ServerHandler implements Handler {
        private NIOService nioService = new NIOService();
        ...
        public void handleRead(SelectionKey key) throws IOException {
            ...
            while(true) {
                ...
                if(readBytes>0) {
                    ...
                    log.info("Server: data = " + msg);

                    String returnMsg = nioService.handleMessage(msg);

                    socketChannel.write(ByteBuffer.wrap(returnMsg.getBytes()));
                    break;
                }
            }
            socketChannel.close();
    ...
    class NIOService {
        public String handleMessage(String message) {
            log.info("service消费消息了");
            return "成功消费!";
        }
    }

    public static void main(String[] args) {
        NIOServer server = new NIOServer("localhost", 8000);
        server.start();
    }
}